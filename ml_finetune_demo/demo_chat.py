...
def parse_args():
    ...
    p.add_argument("--adapter_dir", default=None, help="LoRA 适配器目录，例如 output_lora")
    p.add_argument("--device", type=int, default=-1, help="-1 表示 CPU，0 表示第一块 GPU")
    p.add_argument("--max_new_tokens", type=int, default=80)
    p.add_argument("--temperature", type=float, default=0.7)
    p.add_argument("--top_p", type=float, default=0.9)
    p.add_argument("--do_sample", action="store_true", help="启用采样生成，更像对话")
    p.add_argument("--history_turns", type=int, default=3, help="保留多少轮历史上下文")
    p.add_argument("--log_file", default="data/dialogs.jsonl")
    p.add_argument("--system_prompt", default="你是一个耐心、简洁、中文回答的助手。")
    p.add_argument("--demo", action="store_true", help="运行内置示例并退出")
    return p.parse_args()
