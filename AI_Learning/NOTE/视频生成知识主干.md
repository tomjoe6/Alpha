...
它们都回答“模型怎样从随机性或已生成部分得到完整视觉样本”，可搭配同一种 VAE/tokenizer、DiT 或 U-Net。不要把生成范式和网络骨干混为一谈。

| 范式 | 模型预测什么 | 怎样生成 | 在图像/视频中的位置 |
|---|---|---|---|
| 自回归（AR） | 下一个离散视觉 token 的条件分布 | 按空间、时间或时空顺序逐 token 采样 | 擅长延长与因果视频；长视频 token 多，采样慢且会累积误差 |
| Diffusion / Score | 噪声或带噪分布的 Score | 从噪声经多步去噪 | 当前主流路线；全时空 latent 可联合建模，但传统采样较慢 |
| Flow Matching（FM） | 噪声到数据路径上的速度 | 解 ODE 沿速度场积分 | 与扩散相近的连续生成路线，常面向更直接的少步采样 |
| 一步/少步蒸馏 | 蒸馏后的去噪、速度或区间平均速度 | 一到数步生成 | 是加速目标，不是与扩散/FM 完全并列的基础表示；如 consistency、MeanFlow、Drifting |
| Masked token 生成 | 被遮住的离散 token | 多轮填补 token | 介于 AR 与迭代生成之间，适合离散视觉 token 和局部编辑 |

GAN 与 VAE 也常被称作生成模型，但在现代文生图/视频系统中，GAN 较少作为通用主干；VAE 更常承担压缩与解码。图生视频、文生视频、首尾帧和动作控制则是条件形式，不是生成范式。
