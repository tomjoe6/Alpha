...
生成范式回答“怎样生成”，可搭配任意 VAE/tokenizer 和 U-Net/DiT：

```text
AR：预测下一个 token，逐 token 采样
Diffusion：预测噪声 / Score / x0，通常多步去噪
FM：定义局部速度场；可直接预测 v，也可预测终点 x 后换算，通常多步 ODE 积分
一步生成器：直接输出 x = f(z, c)
```

$x_0$-prediction 仍是扩散的参数化，不等于一步生成。Drifting 是一步生成器的一种训练法：用真实与当前生成分布的差训练 $f$，而不是给每个 $z$ 指定一张真图。VAE 是表示层，图生视频等是条件形式，不要与生成范式混淆。
...
这里 $v$ 是 FM 的数学对象，但不规定网络必须直接输出 $v$。在线性路径下，若网络预测数据终点 $\hat z_1$，可换算为

$$
\hat v_\theta(z_t,t,c)
=\frac{\hat z_1-z_t}{1-t},
\qquad
\hat z_1=z_t+(1-t)\hat v_\theta.
$$

因此 $x$-prediction 与 $v$-prediction 表达同一速度场；但对 $z_1$ 做未加权 MSE 等价于给 $v$-loss 乘 $(1-t)^2$，优化权重和数值条件会改变。某篇论文（如 JiT）报告 $x$-prediction 更好，说明它在该路径、网络和权重下更合适，不说明“FM 预测 $v$ 不好”。
...
> 读法：问题是“一步映射没有 $z\to x$ 配对标签，如何匹配整个数据分布”；创新是用真实 positive 与生成 negative 构造训练期反馈；边界是它并非所有“直接预测 $x$”模型的统称。
