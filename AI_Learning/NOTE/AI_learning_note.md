...
FFN层：一般先对每个向量做升维操作(一般是4倍)，此步可理解为升维矩阵的每一长为h的列在"询问"向量多个问题(由于高维向量都趋于正交,他们之间几乎不会有影响,可以问出的问题是无数个)，得到一个有大小的digital answer ；通过ReLU；最后再降维回原来维数，此步可理解为4h维向量对降维矩阵的每一列加权求和，"总结"之前的digital answer用来加到原向量上(ResNet)得到更精炼的答案。输出的每个维度可以看作一个总结结果。FFN层的作用就是在每个位置上对之前attention得到的向量进行更深入的加工和提炼，得到一个更有信息量的向量。·  

![alt text](image-17.png) 


>参数量分析：Transformer由多个相同层组成，每个层一般包括Self-Attention,MLP,Normalization  
记层数为l，隐藏层维度h，注意力头数a，词表大小V，batch_size为b，序列长度s  
Self-Attention模块包含Q，K，V，FC四个矩阵，每个矩阵为h\*h，总参数量为4h\*h  
MLP模块包含两个FC矩阵，分别为h\*4h和4h\*h，总参数量为8h\*h  
Normalization模块包含两个参数γ和β，总参数量为2h

要注意的是，一开始输入的是embedding维度(隐藏层维数)\*输入长度的矩阵，但是经过attention和FFN后输出是词表大小\*输入长度的矩阵，记录着每个token的概率分布，所以最后还要前乘一个h\*V的矩阵把它投影回词表维度   

>为什么大模型可以储存很多知识?因为在高维空间中,几乎任何向量都趋于正交,所以模型可以在不同的维度上储存不同的知识,而且这些知识之间不会互相干扰,就像在一个巨大的仓库里,每个知识都有自己独立的货架一样。

##### Flash Attention
 
会发现Q换行，O也换行，而V换行则是O的层次叠加  
QK分块会造成softmax的分块，而softmax是按行（某行代表当前token和其他token的相关性）计算的，多创立几个变量保存当前行最大值、已有的元素已经进行的softmax等来进行分过程的online softmax    
